...
void ShareLog::accept(const AcceptEvent* event)
{
	...
	{
		LOG_INFO("#" << event->mapperId() << " accepted (" << m_stats.data().accepted << "/" << m_stats.data().rejected
		         << "+" << m_stats.data().invalid << ") diff " << event->result.diff << " (" << event->result.elapsed <<
		         " ms)");
	}
}
...
void ShareLog::reject(const AcceptEvent* event)
{
	...
	{
		LOG_INFO("#" << event->mapperId() << " rejected (" <<  m_stats.data().accepted << "/" <<
		         m_stats.data().rejected << "+" << m_stats.data().invalid << ") diff " << event->result.diff << " \"" <<
		         event->error() << "\" (" << event->result.elapsed << " ms)");
	}
}
