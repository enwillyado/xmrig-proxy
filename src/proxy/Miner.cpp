...
bool Miner::accept(uv_stream_t* server)
{
	...
	if(rt < 0)
	{
		LOG_ERR("[miner] accept error: \"" << uv_strerror(rt) << "\"");
		return false;
	}
...
bool Miner::parseRequest(int64_t id, const char* method, const rapidjson::Value & params)
{
	...
	if(m_state == WaitLoginState)
	{
		if(strcmp(method, "login") == 0)
		{
			...
			LoginEvent::create(this, id, params["login"].GetString(), params["pass"].GetString(),
			                   params["agent"].GetString())->start();
			return true;
		}
	...
	if(strcmp(method, "submit") == 0)
	{
		...
		SubmitEvent* event = SubmitEvent::create(this, id, params["job_id"].GetString(), params["nonce"].GetString(),
		                     params["result"].GetString());
...
void Miner::parse(char* line, size_t len)
{
	...
	LOG_DEBUG("[" << m_ip << "] received (" << len << " bytes): \"" << line << "\"");
	...
	{
		LOG_ERR("[" << m_ip << "] JSON decode failed: \"" << rapidjson::GetParseError_En(doc.GetParseError()) << "\"");
...
{
	LOG_DEBUG("[" << m_ip << "] send (" << size << " bytes): \"" << m_sendBuf << "\"");
...
void Miner::onRead(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf)
{
	...
	LOG_DEBUG("[" << miner->m_ip << "] read (" << nread << " bytes): \"" << start << "\"");
